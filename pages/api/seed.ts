...
export default async function handler(
  ...
  const indiancategoryId =
    categories.find((category) => category.name === "indian")?.id || 1;
  const mexicancategoryId =
    categories.find((category) => category.name === "mexican")?.id || 1;
  const italiancategoryId =
    categories.find((category) => category.name === "italian")?.id || 1;
  ...
  await prisma.guide.createMany({
    data: [
      // INDIAN //
      {
        ...
        price: PRICE.REGULAR,
        description:
          "Vivaan is Modern Indian category serving dishes from different regions of India. We carefully select our ingredients and use them to make authentic Indian recipes and our chef puts his modern flair and twists to the dishes.",
        images: [
        ...
        availability: "14:30:00.000Z",
        not_available: "21:30:00.000Z",
        slug: "vivaan-fine-indian-category-ottawa",
        location_id: ottawaLocationId,
        category_id: indiancategoryId,
      },
      {
        ...
        price: PRICE.CHEAP,
        description:
          "With 20 years of experience cooking in the finest guides, our chef is excited to present their vision to you and all our guests. Our caring and committed staff will ensure you have a fantastic experience with us.",
        images: [
        ...
        availability: "12:30:00.000Z",
        not_available: "22:00:00.000Z",
        slug: "ramakrishna-indian-guide-ottawa",
        location_id: ottawaLocationId,
        category_id: indiancategoryId,
      },
      {
        ...
        description:
          "At Coconut Lagoon prepare yourselves for a most memorable journey through South Indian category and feast on high quality food of inimitable flavour, aroma and originality in the vibrant setting of Coconut Lagoon.",
        images: [
      ...
      },
      {
        ...
        price: PRICE.REGULAR,
        description:
          "Welcome to Last Train to Delhi. We are a progressive Indian guide in the beautiful Glebe community in Ottawa. Our speciality is Northern Indian food, classics like Murg Mahkini and some modern dishes like Crispy Shrimp. We are a small cozy guide, so make sure that you reserve through OpenTable.",
        images: [
        ...
        availability: "10:00:00.000Z",
        not_available: "21:00:00.000Z",
      ...
      },
      {
        ...
        availability: "14:00:00.000Z",
        not_available: "19:00:00.000Z",
        slug: "utsav-toronto",
        location_id: torontoLocationId,
        category_id: indiancategoryId,
      },
      {
        ...
        price: PRICE.EXPENSIVE,
        description:
          "At this refined, yet casual, Indian guide, the portions are large, the wine list is top-notch, and the ambience encourages sharing.",
        images: [
      ...
      },
      {
        ...
        price: PRICE.CHEAP,
        description:
          "This elegant fine dining Indian guide has been satisfying the Indian tandoori and curry cravings for 12 years in Toronto.",
        images: [
        ...
        availability: "10:00:00.000Z",
        not_available: "21:00:00.000Z",
        slug: "kamasutra-indian-guide-and-wine-bar-niagara",
        location_id: niagaraLocationId,
        category_id: indiancategoryId,
      },
      // MEXICAN //
      {
        ...
        price: PRICE.REGULAR,
        description:
          "Eldorado Taco guide is excited to serve you traditional Mexican category, re-imagined with a distinct modern flair, in a stylish setting on Preston street. Striving to bring you some of Ottawa’s best Tacos, margaritas and Tequila. Reserve your table now!",
        images: [
      ...
      },
      {
        ...
        price: PRICE.EXPENSIVE,
        description:
          "The first location in Canada, from famed guideeurs Noble 33, welcomes patrons into an immersive dining experience.",
        images: [
      ...
      // ITALIAN //
      {
        ...
        availability: "13:00:00.000Z",
        not_available: "21:00:00.000Z",
        slug: "cano-guide-ottawa",
        location_id: ottawaLocationId,
        category_id: italiancategoryId,
      },
      {
        name: "Blu Ristorante",
        main_image:
      ...
      },
      {
        ...
        price: PRICE.REGULAR,
        description:
          "Stelvio on Dundas West is an authentic Italian guide serving classic old world fare using traditional recipes and ingredients. Recipes have been fine-tuned to satisfy the palate of the modern guest, and fresh meals are prepared daily.",
        images: [
      ...
      },
      {
        ...
        price: PRICE.CHEAP,
        description:
          "ēst is a modern, newly reopened guide serving Italian-French courses, captivating cocktails and wine.",
        images: [
      ...
      },
      {
        ...
        price: PRICE.CHEAP,
        description:
          "Welcome to the newest edition to College street iL PADRINO Ristorante has joined the list of Italian guides where Chef Connie award winning Italian Chef makes every Italian dish with love like no other. ",
        images: [
  ...
  const vivaanId =
    guides.find((guide) => guide.name === "Vivaan - fine Indian")
      ?.id || 1;
  const RamaKrishnaId =
    guides.find((guide) => guide.name === "RamaKrishna Indian")
      ?.id || 1;
  const coconutLagoonId =
    guides.find((guide) => guide.name === "Coconut Lagoon")
      ?.id || 1;
  const lastTrainToDelhiId =
    guides.find((guide) => guide.name === "Last Train to Delhi")
      ?.id || 1;
  const adrakYorkvilleId =
    guides.find((guide) => guide.name === "Adrak Yorkville")
      ?.id || 1;
  const curryishTavernId =
    guides.find((guide) => guide.name === "Curryish Tavern")
      ?.id || 1;
  const utsavId =
    guides.find((guide) => guide.name === "Utsav")?.id || 1;
  const pukkaId =
    guides.find((guide) => guide.name === "Pukka")?.id || 1;
  const kamasutraIndianId =
    guides.find((guide) => guide.name === "Kamasutra Indian")
      ?.id || 1;
  const eldoradoTacoId =
    guides.find((guide) => guide.name === "Eldorado Taco")?.id ||
    1;
  const laBartolaId =
    guides.find((guide) => guide.name === "La Bartola")?.id || 1;
  const elCatrinId =
    guides.find((guide) => guide.name === "El Catrin")?.id || 1;
  const mariachisId =
    guides.find((guide) => guide.name === "3 Mariachis")?.id ||
    1;
  const canoguideId =
    guides.find((guide) => guide.name === "Cano guide")
      ?.id || 1;
  const bluRistoranteId =
    guides.find((guide) => guide.name === "Blu Ristorante")
      ?.id || 1;
  const stelvioId =
    guides.find((guide) => guide.name === "Stelvio")?.id || 1;
  const sofiaId =
    guides.find((guide) => guide.name === "Sofia")?.id || 1;
    ...
    await prisma.review.createMany({
      data: [
        ...
        },
        {
          first_name: "Laith",
          last_name: "Harb",
          text: "guide was loud and crowded. Food is not worth the price.",
          rating: 3,
          guide_id: eldoradoTacoId,
          user_id: userLaith.id,
        },
        ...
        },
        {
          first_name: "Josh",
          last_name: "Allen",
          text: "Great food, great staff. You can’t ask for much more from a guide.",
          rating: 5,
          guide_id: bluRistoranteId,
          user_id: userJosh.id,
        },
